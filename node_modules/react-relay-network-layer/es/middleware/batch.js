...
function passThroughBatch(req, next, opts) {
  const {
    singleton
  } = opts; // req.body as FormData can not be batched!
...
function sendRequests(requestMap, next, opts) {
  ...
  if (ids.length === 1) {
    ...
    return next(req).then(batchResponse => {
      ...
      const responseHasIds = payload.every(response => response.id);
...
function finalizeUncompleted(requestMap, e) {
  Object.keys(requestMap).forEach(id => {
    ...
    if (!request.done) {
      request.completeErr(e || new Error(`Server does not return response for request with id ${id} \n` + `Response should have following shape { "id": "${id}", "data": {} }`));
    }
  });
}
